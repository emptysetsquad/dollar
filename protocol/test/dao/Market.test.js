...
describe("Market", function () {
  ...
  beforeEach(async function () {
    this.market = await MockImpl.new(poolAddress, {
      from: ownerAddress,
      gas: 8000000,
    });
  ...
  describe("redeemCoupons", function () {
    beforeEach(async function () {
      ...
      this.couponAmount = 0;
      await this.market.incrementBalanceOfCouponUnderlyingE(
        userAddress,
        1,
        100000
      );
    ...
    describe("before redeemable", function () {
      describe("same epoch", function () {
        it("reverts", async function () {
          await expectRevert(
            this.market.redeemCoupons(1, this.couponUnderlying, {
              from: userAddress,
            }),
    ...
    describe("redeem underlying", function () {
      ...
      beforeEach(async function () {
        ...
        this.result = await this.market.redeemCoupons(
          1,
          this.couponUnderlying,
          { from: userAddress }
        );
      ...
      it("updates user balances", async function () {
        expect(await this.dollar.balanceOf(userAddress)).to.be.bignumber.equal(
          new BN(900000 + this.couponUnderlying)
        );
        expect(
      ...
      it("updates dao balances", async function () {
        ...
        expect(await this.market.totalCouponUnderlying()).to.be.bignumber.equal(
          new BN(0)
        );
        expect(await this.market.totalDebt()).to.be.bignumber.equal(new BN(0));
        expect(await this.market.totalRedeemable()).to.be.bignumber.equal(
          new BN(0)
        );
      });
  ...
  describe("approveCoupons", function () {
    describe("zero address", function () {
      it("reverts", async function () {
        await expectRevert(
          this.market.approveCoupons(ZERO_ADDRESS, 1000, { from: userAddress }),
    ...
    describe("on single call", function () {
      beforeEach(async function () {
        this.result = await this.market.approveCoupons(ownerAddress, 100000, {
          from: userAddress,
        });
        this.txHash = this.result.tx;
      });
      ...
      it("updates user approval", async function () {
        expect(
          await this.market.allowanceCoupons(userAddress, ownerAddress)
    ...
    describe("multiple calls", function () {
      beforeEach(async function () {
        ...
        this.result = await this.market.approveCoupons(ownerAddress, 0, {
          from: userAddress,
        });
        this.txHash = this.result.tx;
      });
  ...
  describe("transferCoupons", function () {
    ...
    describe("sender zero address", function () {
      it("reverts", async function () {
        await expectRevert(
          this.market.transferCoupons(ZERO_ADDRESS, userAddress, 1, 100000, {
            from: userAddress,
          }),
    ...
    describe("recipient zero address", function () {
      it("reverts", async function () {
        await expectRevert(
          this.market.transferCoupons(userAddress, ZERO_ADDRESS, 1, 100000, {
            from: userAddress,
          }),
    ...
    describe("on call from self", function () {
      ...
      it("updates balances", async function () {
        expect(
          await this.market.balanceOfCoupons(userAddress, 1)
        ).to.be.bignumber.closeTo(new BN(0).divn(2), new BN(1));
        expect(
          await this.market.balanceOfCouponUnderlying(userAddress, 1)
        ).to.be.bignumber.equal(new BN(50000));
        expect(
          await this.market.balanceOfCoupons(ownerAddress, 1)
        ).to.be.bignumber.closeTo(new BN(0).divn(2), new BN(1));
        expect(
          await this.market.balanceOfCouponUnderlying(ownerAddress, 1)
        ).to.be.bignumber.equal(new BN(50000));
      });
    ...
    describe("on call from self too much", function () {
      it("reverts", async function () {
        await expectRevert(
          this.market.transferCoupons(userAddress, ownerAddress, 1, 200000, {
            from: ownerAddress,
          }),
    ...
    describe("on unapproved call from other", function () {
      it("reverts", async function () {
        await expectRevert(
          this.market.transferCoupons(userAddress, ownerAddress, 1, 100000, {
            from: ownerAddress,
          }),
    ...
    describe("on approved call from other", function () {
      beforeEach(async function () {
        await this.market.approveCoupons(ownerAddress, 50000, {
          from: userAddress,
        });
        this.result = await this.market.transferCoupons(
          userAddress,
