...
describe('Regulator', function () {
  ...
  beforeEach(async function () {
    ...
    this.regulator = await MockRegulator.new(this.oracle.address, poolAddress, treasury, {from: ownerAddress, gas: 8000000});
    ...
    describe('above limit', function () {
      ...
      describe('on step', function () {
        ...
        it('mints new Dollar tokens', async function () {
          ...
          expect(await this.dollar.balanceOf(this.regulator.address)).to.be.bignumber.equal(lessAllIncentive(1000000, 100000));
          expect(await this.dollar.balanceOf(poolAddress)).to.be.bignumber.equal(poolIncentive(100000));
          expect(await this.dollar.balanceOf(treasury)).to.be.bignumber.equal(treasuryIncentive(100000));
        });

        it('updates totals', async function () {
          expect(await this.regulator.totalStaged()).to.be.bignumber.equal(new BN(0));
          expect(await this.regulator.totalBonded()).to.be.bignumber.equal(lessAllIncentive(1000000, 100000));
    ...
    describe('(3) - only to bonded', function () {
      ...
      describe('on step', function () {
        ...
        it('mints new Dollar tokens', async function () {
          ...
          expect(await this.dollar.balanceOf(this.regulator.address)).to.be.bignumber.equal(lessAllIncentive(1000000, 50000));
          expect(await this.dollar.balanceOf(poolAddress)).to.be.bignumber.equal(poolIncentive(50000));
          expect(await this.dollar.balanceOf(treasury)).to.be.bignumber.equal(treasuryIncentive(50000));
        });

        it('updates totals', async function () {
          expect(await this.regulator.totalStaged()).to.be.bignumber.equal(new BN(0));
          expect(await this.regulator.totalBonded()).to.be.bignumber.equal(lessAllIncentive(1000000, 50000));
          expect(await this.regulator.totalDebt()).to.be.bignumber.equal(new BN(0));
          expect(await this.regulator.totalSupply()).to.be.bignumber.equal(new BN(0));
          expect(await this.regulator.totalCoupons()).to.be.bignumber.equal(new BN(0));
          expect(await this.regulator.totalRedeemable()).to.be.bignumber.equal(new BN(0));
    ...
    describe('(2) - only to repay debt', function () {
      ...
      describe('on step', function () {
        ...
        it('doesnt mint new Dollar tokens', async function () {
          ...
          expect(await this.dollar.balanceOf(treasury)).to.be.bignumber.equal(new BN(0));
  ...
  describe('(2 + 3) - repay debt then mint to bonded', function () {
    ...
    describe('on step', function () {
      ...
      it('mints new Dollar tokens', async function () {
        ...
        expect(await this.dollar.balanceOf(this.regulator.address)).to.be.bignumber.equal(lessAllIncentive(1000000, 39500));
        expect(await this.dollar.balanceOf(poolAddress)).to.be.bignumber.equal(poolIncentive(39500));
        expect(await this.dollar.balanceOf(treasury)).to.be.bignumber.equal(treasuryIncentive(39500));
      });

      it('updates totals', async function () {
        expect(await this.regulator.totalStaged()).to.be.bignumber.equal(new BN(0));
        expect(await this.regulator.totalBonded()).to.be.bignumber.equal(lessAllIncentive(1000000, 39500));
        expect(await this.regulator.totalDebt()).to.be.bignumber.equal(new BN(0));
        expect(await this.regulator.totalSupply()).to.be.bignumber.equal(new BN(0));
        expect(await this.regulator.totalCoupons()).to.be.bignumber.equal(new BN(0));
        expect(await this.regulator.totalRedeemable()).to.be.bignumber.equal(new BN(0));
  ...
  describe('(1 + 2 + 3) - refresh redeemable then repay debt then mint to bonded', function () {
    ...
    describe('on step', function () {
      ...
      it('mints new Dollar tokens', async function () {
        expect(await this.dollar.totalSupply()).to.be.bignumber.equal(new BN(1039500));
        expect(await this.dollar.balanceOf(this.regulator.address)).to.be.bignumber.equal(lessAllIncentive(1010000, 29500));
        expect(await this.dollar.balanceOf(poolAddress)).to.be.bignumber.equal(poolIncentive(29500));
        expect(await this.dollar.balanceOf(treasury)).to.be.bignumber.equal(treasuryIncentive(29500));
      });

      it('updates totals', async function () {
        expect(await this.regulator.totalStaged()).to.be.bignumber.equal(new BN(0));
        expect(await this.regulator.totalBonded()).to.be.bignumber.equal(lessAllIncentive(1000000, 29500));
        expect(await this.regulator.totalDebt()).to.be.bignumber.equal(new BN(0));
        expect(await this.regulator.totalSupply()).to.be.bignumber.equal(new BN(0));
        expect(await this.regulator.totalCoupons()).to.be.bignumber.equal(new BN(10000));
        expect(await this.regulator.totalRedeemable()).to.be.bignumber.equal(new BN(10000));
